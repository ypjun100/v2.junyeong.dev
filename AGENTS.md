...
| `script/` | 폰트 서브셋 생성, 사진 게인맵 제거. 사이트 빌드와는 무관하다 |
| `imgs/` | 글이 참조하는 이미지 |
...
아이폰으로 찍은 사진은 기본 이미지 뒤에 **게인맵**이 하나 더 붙어 온다. MPF 세그먼트(APP2)로 연결된 두 번째 이미지이고, HDR 디스플레이에서 iOS Safari가 이걸 적용해 밝기를 끌어올린다. 페이지는 그대로인데 사진만 하얗게 뜨면 이것이다. 파일 안의 데이터라 CSS로는 끌 수 없다.

`imgs/`에 새 사진을 넣었으면 확인한다. 글을 쓸 때마다 돌릴 필요는 없고, 사진을 추가했을 때만 보면 된다.

```bash
python3 script/strip-gain-maps.py            # 대상만 보여준다
python3 script/strip-gain-maps.py --write    # 잘라낸다
```

**`jpegtran -copy none`은 쓰지 않는다.** 게인맵과 함께 EXIF와 ICC 프로파일까지 날아간다. 이 사진들은 Display P3 프로파일을 갖고 있고 그중 하나는 EXIF Orientation에 의존해 똑바로 선다. 스크립트는 MPF 세그먼트와 뒤에 붙은 이미지만 잘라내며, 쓰기 전에 기본 이미지의 압축 스캔이 그대로인지와 나머지 세그먼트가 남아 있는지를 스스로 검사한다.

## 하지 말 것

- 목록을 템플릿에 하드코딩하지 않는다. 목록은 항상 컬렉션에서 나온다.
- 글 본문을 HTML로 옮기지 않는다.
- npm이나 번들러 같은 빌드 툴체인을 도입하지 않는다. `script/`의 두 스크립트는 사이트 빌드가 아니라 에셋 준비용이며 배포 경로에 들어가지 않는다.
- `_site/`를 커밋하지 않는다.
