...
| `imgs/` | 글이 참조하는 이미지 |
| `docs/specs/` | 작업 스펙. `.gitignore`로 빠져 있어 로컬에만 있고, 사이트에도 배포되지 않는다 |
...
- `content/`의 이름을 바꾸거나 밑줄을 붙이면 블로그 글 19편이 조용히 사라집니다. 위 디렉터리 절의 설명을 참고하세요.
- 기존 글의 파일명을 바꾸면 주소가 바뀌어 외부 링크가 깨집니다.
- 빌드 성공은 콘텐츠가 다 실렸다는 뜻이 아닙니다. 항상 페이지 개수를 대조하세요.
- 사이드바나 섹션 라벨에 새 한글을 넣고 폰트를 재생성하지 않으면 그 글자만 다른 서체로 나옵니다. 본문 글에는 해당하지 않습니다. 위 폰트 절을 보세요.
- 아이폰 사진을 `imgs/`에 그대로 넣으면 게인맵이 딸려 들어와 아이폰에서 그 사진만 하얗게 뜹니다. CSS로는 못 막습니다. 위 이미지 절을 보세요.
