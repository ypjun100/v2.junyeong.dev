...
2. `.github/workflows/pages.yml`
   ...
   - build 잡: `actions/checkout` → `ruby/setup-ruby`(`bundler-cache: true`) → `actions/configure-pages` → `bundle exec jekyll build --strict_front_matter` → `bundle exec htmlproofer _site --disable-external --allow-hash-href --ignore-missing-alt` → `actions/upload-pages-artifact`
