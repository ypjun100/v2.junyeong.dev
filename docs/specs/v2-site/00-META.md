...
- 2026-08-23 후속 리뷰(직전 리뷰 이후 커밋 4개 포함 전체 diff 재검토): 라이트박스 키보드 개선이 포인터 세션의 닫기에서 원본 이미지에 포커스 링을 되살리던 것을 세션 단위 억제로 고쳤고, CI에 개수 대조와 PR 빌드를 추가했으며, 스펙 문서의 이력 서술을 현재 동작 서술로 정리했다.
