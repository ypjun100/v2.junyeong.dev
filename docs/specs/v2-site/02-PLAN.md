...
본문의 강조는 세 컬렉션 모두 `**굵게**`만 쓴다. `<mark>`(형광펜)와 백틱(코드 배경)을 섞으면 한 문단에 강조 방식이 세 가지가 되고, 프로젝트·수상 경력의 백틱은 코드가 아니라 용어 강조였다. 백틱 제약은 코드를 실제로 다루는 블로그 글에는 적용하지 않는다. `s`와 `mark`는 스타일이 없어 본래 의미대로 렌더링된다.
...
`content` 안의 마크다운은 `_post.scss`가 요소 선택자로 스타일링한다. 시안이 각 요소에 붙여 둔 인라인 스타일을 `h2`, `p`, `pre`, `code`, `ul`, `strong`, `em`, `blockquote`, `img`, `table` 선택자로 옮긴다. 기존 글이 `>` 인용을 자주 쓰므로 인용문은 반드시 다크 톤 스타일을 갖는다.
...
- 블로그: 19편 중 11편이 `<s>`를 형광펜으로 바꾸는 인라인 `<style>` 블록을 갖고 있다. 이 블록을 지우고 `<s>` 강조는 `**굵게**`로 바꾼다. 그 외의 본문 문장은 건드리지 않는다.
